...
async function assertOwner(admin: ReturnType<typeof supabaseAdmin>, poolId: string, userId: string) {
  ...
  if (!data || data.role !== "owner") {
    return { ok: false as const, error: "Forbidden (owner only)" };
  }
...
export async function DELETE(
  ...
  try {
    ...
    // 🔥 1. Delete predictions for this pool
    await admin
      .from("predictions")
      .delete()
      .eq("pool_id", poolId);

    // 🔥 2. Delete pool members
    await admin
      .from("pool_members")
      .delete()
      .eq("pool_id", poolId);

    // 🔥 3. Delete pool itself
    const { error: delPoolErr } = await admin
      .from("pools")
      .delete()
      .eq("id", poolId);

    if (delPoolErr) {
      return NextResponse.json({ error: delPoolErr.message }, { status: 500 });
    }
