...
export default function WeekendBonusPage() {
  ...
  const backHref = useMemo(() => `/pools/${poolId}/event/${eventId}`, [poolId, eventId]);
  ...
  async function load() {
    ...
    const res = await fetch(`/api/pools/${poolId}/event/${eventId}/bonus`, {
  ...
  return (
    <main style={{ padding: 16, maxWidth: 900 }}>
      <Link href={backHref}>← Terug naar weekend</Link>

      <h1 style={{ marginTop: 12 }}>Weekend bonus</h1>
      <p style={{ opacity: 0.8, marginTop: 6 }}>
        Dit weekend krijg je 3 random bonusvragen. Deze locken bij de start van de eerste sessie van dit weekend.
      </p>
      ...
      {questions.length === 0 ? (
        <p style={{ marginTop: 14 }}>
          Nog geen weekend bonus vragen. (Admin moet ze toevoegen in bonus_questions)
        </p>
      ) : (
